...
import {minChainBalance, minEvmStableBalance} from "./config.js";
import {
    approveToken,
...
export async function bridgeToAptos(evmKey, aptosKey, chain, stableToken) {
    ...
    try {
        while (usdtBalance < minEvmStableBalance) {
            const sleepTime = random(30, 100);
            logger.warn(`waiting for USDT bridge from BINANCE -  ${sleepTime} seconds`)
